...
class KitsuApi(private val client: OkHttpClient, interceptor: KitsuInterceptor) {
    ...
    private suspend fun algoliaSearchAnime(key: String, query: String): List<AnimeTrackSearch> {
        return withIOContext {
            val jsonObject = buildJsonObject {
                put("params", "query=${URLEncoder.encode(query, StandardCharsets.UTF_8.name())}$ALGOLIA_FILTER_ANIME")
            }
    ...
    companion object {
        ...
        private const val ALGOLIA_FILTER_ANIME = "&facetFilters=%5B%22kind%3Aanime%22%5D&attributesToRetrieve=" +
